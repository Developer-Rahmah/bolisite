...
class HealthInsuranceQuestion extends Component{
    constructor(props){
        super(props);
        this.state ={
...
arr.push(questions[i])
    ...
    if(arr.length<=0){
    this.props.goFromHealthInsuranceQuestion(questions,this.props.user_id,this.props.healthInfo)
    }
    else{
      this.dropdown.alertWithType('error',strings('message.error',lang), strings('message.fill_message',lang));
      ...
      const {lang,questions}=this.props
        ...
        <View style={{backgroundColor:'transparent',borderColor:'transparent'}}>
                     ...
                     <View style={{marginRight:lang=='ar'?20:null,marginLeft:lang=='en'?20:null,marginTop:20}} key={item.id}>
                          ...
                          <Label style={{textAlign:lang=='ar'?"right":"left",color:this.state.form_submitted==true&&item.Yes==false&&item.No==false?"red":"#171717",fontFamily:'TajawalBold0',lineHeight:25}}>{item.question}</Label>
                      </View>
 ...
 <Label style={[labelStyle,{textAlign:lang=='ar'?"right":"left",color: "#171717",marginBottom:10}]}>{strings('health_insurance.no',lang)}</Label>
      ...
      {/* </Right> */}
           ...
           onPress={this.goFromHealthInsuranceQuestion}>               
...
const mapStateToProps = state => {
  ...
  export default connect(mapStateToProps,healthInsuranceAction)
